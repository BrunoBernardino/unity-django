...
/* json2.js | 2012-10-08 | https://raw.github.com/douglascrockford/JSON-js/master/json2.js */
...
/*
 * :data() selector for jQuery
 * If the above doesn't do the trick, you can always use native jQuery, e.g.: $('div').filter(':data', function(data){ return 'dataAttribute' in data;});
 */
/*(function($){
...
(function($) {
	...
	var _getJSONDate = function(dateStr) {
		...
		if (typeof dateStr == "string") {
			...
			//-- "2012-09-18T23:00:00+00:00"
			var isoRegDjango = /^([0-9]{4})-([0-9]{2})-([0-9]{2})T([0-9]{2}):([0-9]{2}):([0-9]{2})\+([0-9]{2}):([0-9]{2})$/;
			...
			if (k = dateStr.match(isoRegDjango)) {
				return new Date(k[1],k[2]-1,k[3],k[4],k[5],k[6]);
			} else if (k = dateStr.match(isoRegPlus)) {
				return new Date(k[1],k[2]-1,k[3],k[4],k[5],k[6]);
	...
	var _filterEventCollection = function() {
		if (cEvents && cEvents.length > 0) {
			...
			$.each(cEvents, function(){
				...
				if(ev.eDate) {
					if (typeof ev.eDate == 'object' && ev.eDate.getDate) { this.eDate = ev.eDate; }
					if (typeof ev.eDate == 'string' && ev.eDate.split) { this.eDate = _getJSONDate(ev.eDate); }
				} else {
	...
	var _eventSort = function(a, b) {
		return a.eDate.compareTo(b.eDate);
	};
	...
	var _initHeaders = function() {
		...
		var prevMLink = $('<div class="MonthNavPrev"><a class="link-prev btn btn-small">'+ def.navLinks.p +'</a></div>').click(function() {
		...
		var nextMLink = $('<div class="MonthNavNext"><a class="link-next btn btn-small">'+ def.navLinks.n +'</a></div>').click(function() {
		...
		if(def.navLinks.enablePrevYear) {
			prevYLink = $('<div class="YearNavPrev"><a class="btn btn-small">'+ prevYear.getFullYear() +'</a></div>').click(function() {
		...
		if(def.navLinks.enableNextYear) {
			nextYLink = $('<div class="YearNavNext"><a class="btn btn-small">'+ nextYear.getFullYear() +'</a></div>').click(function() {
		...
		if(def.navLinks.enableToday) {
			...
			todayLink = $('<div class="TodayLink"><a class="link-today btn btn-primary">'+ def.navLinks.t +'</a></div>').click(function() {
				//var now = new Date()
				//$.J.ChangeMonth(now);
	...
	$.J.DrawCalendar = function(dateIn){
		...
		for (var i = 0; i < _totalBoxes; i++) {
			...
			dateLink.on('click', { Date: currentDate.clone() }, def.onDayLinkClick);
			
			var dateBox = $("<td></td>").attr(atts).append(dateLink);
			dateBox.on('dblclick', { Date: currentDate.clone() }, def.onDayCellDblClick);
			dateBox.on('click', { Date: currentDate.clone() }, def.onDayCellClick);
	...
	var _drawEventsOnCalendar = function() {
		...
		if (cEvents && cEvents.length > 0) {
			...
			$.each(cEvents, function(){
				...
				var tempStartDT = ev.eDate.clone().clearTime();
				var tempEndDT = ev.eDate.clone().clearTime();
				...
				for (var i = istart; i <= iend; i++) {
					...
					if (startBoxCompare == 0 || (i == 0 && startBoxCompare == -1)) {
						...
						var e = _eventObj['event_' + ev.id + '_' + b.weekNumber];
						if (e) {
							var w = e.css("width")
	...
	var _buildEventBlock = function(ev, weekNumber) {
		var block = $('<div class="event" id="event_' + ev.id + '_' + weekNumber + '" eventid="' + ev.id +'" title="' + ev.title + '"></div>');
		
		if(ev.cssClass) { block.addClass(ev.cssClass) }
//		block.on('click', { event: ev }, def.onEventBlockClick);
		block.on('mouseover', { event: ev }, def.onEventBlockOver);
		block.on('mouseout', { event: ev }, def.onEventBlockOut);
		
		var link;
		link = $('<a href="' + ev.url + '" target="_blank">' + ev.name + '</a>');
		
//		link.on('click', { event: ev }, def.onEventLinkClick);
		block.append(link);
		return block;
	}
...
(function($, undefined) {
  ...
  $.ajaxTransport("iframe", function(options, origOptions, jqXHR) {
    ...
    if (files.length) {
      ...
      //-- Django CSRF Token
      $("<input type='hidden' />").attr({name: 'csrfmiddlewaretoken', value: $.cookie('csrftoken')}).appendTo(form);
